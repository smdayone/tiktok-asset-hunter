...
# ── 1. Verify D:\ drive ───────────────────────────────────────────────────────
if (-not (Test-Path "D:\")) {
    Write-Host "  [✗] Drive D:\ not found. Connect your external SSD and try again." -ForegroundColor Red
    exit 1
}
Write-Host "  [✓] Drive D:\ detected." -ForegroundColor Green
Write-Host ""

# ── 2. Keyword ────────────────────────────────────────────────────────────────
$keyword = Read-Host "  Keyword (e.g. wireless earbuds)"
if ([string]::IsNullOrWhiteSpace($keyword)) {
    Write-Host "  [✗] Keyword cannot be empty." -ForegroundColor Red
    exit 1
}
...
$linksFile    = if ([string]::IsNullOrWhiteSpace($linksInput)) { $defaultLinks } else { $linksInput }

if (-not (Test-Path $linksFile)) {
    Write-Host "  [✗] Links file not found: $linksFile" -ForegroundColor Red
    exit 1
}
...
$maxVideos  = if ([string]::IsNullOrWhiteSpace($maxInput)) { $defaultMax } else { [int]$maxInput }
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "  [✗] Download script exited with error code $LASTEXITCODE." -ForegroundColor Red
    exit $LASTEXITCODE
}
